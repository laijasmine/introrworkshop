...
ttow <- read.csv("../data/The_Fellowship_Of_The_Ring.csv")

rking <- read.csv("../data/The_Fellowship_Of_The_Ring.csv")
```
...
`spread()` is another tidyr function that converts a dataframe from the long
...
```{r}
cases_tidy <- cases %>% 
  gather(key = "Year", value = "TB_cases", X2011:X2013) %>% 
  group_by(country) %>% 
